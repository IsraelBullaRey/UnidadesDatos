...
public class ListaCD <T> implements Iterable<T>
{
    ...
    */	      
    ...
    private NodoD<T> getPos(int i)throws RuntimeException{
        if(i<0||i>=this.tamanio){
            throw new RuntimeException("El índice solicitado no existe en la Lista Doble");
        }
        else
        {
     ...
     */
    @SuppressWarnings("empty-statement")
    private NodoD<T> getPos_optimizado(int pos)throws RuntimeException{
       if (pos >= 0 && pos < this.tamanio) {
        } else {
           throw new RuntimeException("La posición " + pos + " No es válida en la lista");      
        }
